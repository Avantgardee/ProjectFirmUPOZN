...
Var
    ...
    Emp: TEmployee;
Begin
    //Если число колонок в таблцие равно 5
    If strgrdList.ColCount = 5 Then
    Begin
    //Получить ответ от модального окна добавления записи о сотруднике
        Res := WrkForm.ShowForNew(Emp);
    //Если ответ 'отмена'
        If Res = MrCancel Then
    //Выйти из процедуры
            Exit;
    //Если нет, то проверить на существование сотрудника, который полностью идентичен добавляемому
        If FormMain.EmployeeList.EmployeeExists(Emp) Then
            ShowMessage('Сотрудник уже существует!')
    //Проверить на существование руководителя с таким же кодом
        Else If FormMain.EmployeeList.WorkerCodeExists(Emp.Code) Then
            ShowMessage('Код сотрудника уже существует!')
    //Проверить, существует ли руководитель с таким кодом
    ...
    //Если все проверки пройдены, то добавить сотрудника
        Else
        Begin
...
Var
    ...
    Emp: TEmployee;
Begin
    //Если номер выбранной строки больше 0
    If strgrdList.Row > 0 Then
    Begin
    //Если отображается таблица для сотрудников
        If strgrdList.ColCount = 5 Then
        Begin
    //Получение данных у выбранной строки
            Emp := FormMain.EmployeeList.FindPrjct
              (strgrdList.Cells[0, strgrdList.Row]);
    //Если удаляемый сотрудник - руководитель
            If Emp.Post = 'Руководитель' Then
            Begin
    //Удалить закрепленные за ним задачи и сотрудников
                FormMain.EmployeeList.DeleteWorker(Emp.Code);
                FormMain.WorkList.DeleteDir(Emp.Code)
            End
    //Если удаляемый - обычный сотрудник
            Else
    //Удалить данные о нём
                FormMain.WorkList.DeleteWork(Emp.Code);
            FormMain.EmployeeList.Delete(Emp);
        End
    //Если отображается таблица задач
        Else
        Begin
